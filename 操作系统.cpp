...
customer * read_data_customer()//用来读取顾客数据信息
{
	FILE *fp;
	char word;
	customer *p = NULL;
...
orders *read_data_order()//用来读取订单数据信息
{
	FILE *fp;
	char word1;
	char word2;
...
lineitem *read_data_lineitem()//用来读取商品信息
{
	FILE *fp;
	char word1;
	char word2;
...
result *select(customer *cus, orders *ord, lineitem *lin,char *order_date,char *ship_date)//根据题目所给条件对符合要求的数据信息进行筛选
{
	result *flag1=NULL;
	result *flag2=NULL;
	int control = 0;
...
int main(int argc,char **argv)//实现命令行传参
{
